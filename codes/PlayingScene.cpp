...
void PlayingScene::Draw()
{
	...
	if (!isPaused)
	{
		...
		AnglePercent = (elapsedSec / GAME_TIME) * 100.0;
	}

	if (AnglePercent > 100.0) AnglePercent = 100.0;			// 最大100%に制限
	if (playState == MAIN)
	{
		if (remainingTime <= 10)
		{
			DrawCircleGauge(600, 40, AnglePercent, imgRedTimer);	// 残り時間10秒以下は赤いゲージ描画
		}
		else
		{
			DrawCircleGauge(600, 40, AnglePercent, imgTimer);		// ゲージ描画
		}
	}
	...
	SetDrawBright(255, 255, 255);	// 画像の明るさが戻らなかった場合の保険
	...
	DrawScrollingPauseBackground(bgX, gWindow.width, gWindow.height, imgPauseBackground);
	SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);	// ブレンドモードをリセット
